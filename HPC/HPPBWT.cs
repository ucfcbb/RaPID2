...
using Range = RaPIDv2_HPC_1._0.Utl.Range;

namespace RaPIDv2_HPC_1._0
{
    public class HPPBWT
    {
        public static List<PDA>[] PDA_Holders;
        static int[][] psHolder_MX;
        static short[][] minDZ_MX;
        static int[][] offsetsHolder_MX;
        ...
        public HPPBWT()
        {
            ...
            PDA_Holders = new List<PDA>[Program.nRDP];


            for (int r = 0; r < Program.nRDP; r++)
            {
                PDA_Holders[r] = new List<PDA>(Program.rdp.windows.Count);
                for (int w = 0; w < Program.rdp.windows.Count; w++)
                {
                    PDA_Holders[r].Add(new PDA(Program.panel.nHap));
                }
            }


            psHolder_MX = new int[Program.nRDP][];
            for (int i = 0; i < Program.nRDP; i++)
            {
                psHolder_MX[i] = new int[Program.panel.nHap];
            }

            offsetsHolder_MX = new int[Program.nRDP][];
            for (int i = 0; i < Program.nRDP; i++)
            {
                offsetsHolder_MX[i] = new int[Program.nThread_PD];
            }


            minDZ_MX = new short[Program.nRDP][];
            for (int i = 0; i < Program.nRDP; i++)
            {
                minDZ_MX[i] = new short[Program.nThread_PD];
            }
            //minDZ_Ready = new bool[Program.nThread_PD];
        ...
        void coreP_Arr(int RDP_ID, int wndIndex)
        {
            ...
            PDA newPDA = PDA_Holders[RDP_ID][wndIndex];
            PDA oldPDA = PDA_Holders[RDP_ID][wndIndex - 1];
            ...
            int[] psHolder = psHolder_MX[RDP_ID];
            int[] offsetsHolder = offsetsHolder_MX[RDP_ID];
            ...
            Parallel.For(0, Program.panel.nHap, op, (i) =>
            {
                if (oneSite[oldPDA.pArr[i]] == false)
                {
                    ...
                    //newPDA.revPA[oldPDA.pArr[i]] = i - psHolder[i];

                }
                else
                {
                    newPDA.pArr[psHolder[i] + oneOff] = oldPDA.pArr[i];
                    //newPDA.revPA[oldPDA.pArr[i]] = psHolder[i] + oneOff;
                }
            });
            #endregion
        ...
        void InitialSort(int RDP_ID)
        {
            int[] psHolder = psHolder_MX[RDP_ID];
            int[] offsetsHolder = offsetsHolder_MX[RDP_ID];
            PDA oldPDA = PDA_Holders[RDP_ID][0];
        ...
        void PD_Arr(int RDP_ID)
        {
            int[] selectedSites = Program.rdp.Get_SelectedSites_ByThread(RDP_ID);

            InitialSort(RDP_ID);

            for (int w = 1; w < selectedSites.Length; w++)
            {
                coreP_Arr(RDP_ID, w);
                coreD_Arr(RDP_ID, w);
            }
        ...
        public void Run_PD()
        {
            ...
            Console.WriteLine(DateTime.Now + " HP-PBWT Preload P&D...");
            ParallelOptions pop = new ParallelOptions();
            pop.MaxDegreeOfParallelism = Program.nInstanceHP_PBWT;
            Parallel.For(0, Program.nRDP, pop, (r) =>
            //for (int r = 0; r < Program.nRDP; r++)
            {
                //Console.WriteLine(DateTime.Now + " HP-PBWT " + r + "/" + Program.nRDP);
                PD_Arr(r);
            });
            Console.WriteLine(DateTime.Now + " HP-PBWT P&D Loaded.");

        }


        public class LM_VO_v3
        {
            static int nBucket;
            ...
            {
                ParallelOptions op = new ParallelOptions();
                op.MaxDegreeOfParallelism = Program.nInstanceLM;
                ...
                {
                    Console.WriteLine(DateTime.Now + " Partition: " + Program.currentPartition + "/" + Program.nPartition + " ...");
                    Parallel.For(0, Program.nRDP, op, (rID) =>
                    //for (byte rID = 0; rID < Program.nRDP; rID++)
                    {//gather from LM by each RDP
                        byte r = (byte)rID;

                        for (int w = Program.rdp.LM_StartWndID; w < Program.rdp.windows.Count - 1; w++)
                        {

                            CoreL(r, w);
                        }
                        CoreL_Tail(r, Program.rdp.windows.Count - 1);
            ...
            {
                //Console.WriteLine(DateTime.Now + " R: " + RDP_ID + " L: " + Wnd_ID);
                //Stopwatch spw = new Stopwatch();

                //spw.Start();

                PDA oldPDA = PDA_Holders[RDP_ID][Wnd_ID];
            ...
            {
                //Console.WriteLine(DateTime.Now + " R: " + RDP_ID + " LT");
                PDA oldPDA = PDA_Holders[RDP_ID][Wnd_ID];
